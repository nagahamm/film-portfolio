...
## Media Aspect Ratio Rules

1. **インライン(ページ上)表示**:
   - 担当領域「camera」以外: 縦スクロール量の増加を抑えるため `aspect-ratio: 4 / 5; object-fit: cover;` に固定する(`.ep-media`, `.craft-card.has-photo`, `.shotlist-track img` 等)。
...
1. **1コミット=1タスク**: 「なぜこの変更をしたか」の理由が1つで説明しきれる単位に分割すること。複数の目的が混ざったコミットは作らない。
   - 性質の異なる変更(バグ修正 / リファクタリング / 新機能 / パフォーマンス / ドキュメント)は、同じファイルを触る場合でも必ず別コミットに分ける。
   - 逆に、同種のタスクや機械的に連動する変更(例: クラス名リネームに伴う HTML・CSS・JS の同時修正)は1コミットにまとめてよい。ファイル単位ではなくタスク単位で判断する。
2. **各コミットが単体で成立すること**: どのコミット時点でチェックアウトしてもページが壊れず、構文エラーが無い状態を保つこと。「次のコミットで直る」前提の中間状態を残さない。
3. **バグ修正を先、リファクタリングを後**: 両方を行う場合は、原因と対処が追える順に並べる。リファクタリングの巨大な差分に埋もれさせない。
4. **大きな作業は着手前に分割方針を決める**: 全面的な書き換えやリファクタリングでは、作業を始める前にコミットの分割単位を決めてから進めること。作業後にまとめてコミットし、後から分割し直す手戻りを防ぐ。
5. **`git commit --amend` の使い分け**: 直前のコミットの修整・補足であれば amend で統合する(Interaction & Execution Policy 4 を参照)。別タスクであれば必ず新規コミットにする。
